...
import 'package:myacademy_apps/Quiz/Data/quiz_flutter_list.dart';
import 'package:myacademy_apps/Quiz/Screen/hasil_quiz_flutter.dart';

class QuizFlutterPages extends StatefulWidget {
  const QuizFlutterPages({super.key});

  @override
  State<QuizFlutterPages> createState() => _QuizFlutterPagesState();
}

class _QuizFlutterPagesState extends State<QuizFlutterPages> {
            ...
            return Column(
              ...
              children: [
                SizedBox(
                  ...
                  child: Text(
                    'Quiz Flutter ${index + 1}/${question.length}',
                    style: TextStyle(
                      color: Colors.white,
                      fontSize: 28.0,
                    ...
                    OutlinedButton(
                      onPressed: isPressed ? index + 1 == question.length ? () {
                        Navigator.push(
                          context, 
                          MaterialPageRoute(
                            builder: (context) => HasilQuizFlutter(score),
                          ),
                        );
                      }
                      : () {
